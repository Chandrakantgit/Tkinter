...
pygame 
pygame can be installed by : pip install pygame

-------------------------------------------------------------------------------
Description : 
Musicxy MP3 Player is a python tkinter based music player which you can use to
...
class Player(tk.Frame):
	...
	def retrieve_songs(self):
		...
		for root_, dirs, files in os.walk(directory):
				for file in files:
					if (os.path.splitext(file)[1] == '.ogg') or (os.path.splitext(file)[1] == '.wav') or (os.path.splitext(file)[1] == '.mpeg'):
	...
	def play_song(self, event=None):
		...
		self.list.itemconfigure(self.current, bg='orange')
